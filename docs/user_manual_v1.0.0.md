# CROCOEXP User Manual v1.0.0

## Purpose

CROCOEXP v1.0.0 is a reproducible host-side orchestrator for CROCO experiment workflows. It records how an experiment was set up, which source tree was selected, what was imported, what was compiled, what was planned, what was run, and what failed.
...
Out-of-scope execution for v1.0.0:
...
- `CROCO_EXPERIMENTS/sources/<source_id>/`: managed copy of a CROCO, MSOT, or custom source tree.
...
Use this v1.0.0 command sequence:
...
`croco.in` is recorded as evidence, but CROCOEXP does not treat it as universal semantic truth. CROCO input syntax varies across CROCO versions, MSOT, and custom forks. CROCOEXP does not infer required runtime data assets solely from text in `croco.in`.
...
```bash
./crocoexp source inspect croco-msot-local
./crocoexp source inspect croco-msot-local --json
```
...
./crocoexp import minimal --source croco-msot-local
./crocoexp import minimal --json
```

Inputs:

- `CROCO_EXPERIMENTS/<experiment_name>/input/`
- optional registered source ID
...
Unsupported in v1.0.0 unless explicitly implemented and tested:
...
- **Missing compile source**: import with `--source <source_id>` or ensure the manifest has `compile_time.source_ref`.
...
- **Dry-run blocked because unsupported profile**: serial and tested OpenMP are supported; MPI, OPENACC, XIOS, OASIS, and AGRIF are blocked in v1.0.0.
...
Cite CROCOEXP when using it to organize, compile, dry-run, run, or document CROCO experiments. Cite the exact version used. For v1.0.0, the citation metadata source of truth is `CITATION.cff`.
...
Before relying on a v1.0.0 run record:
